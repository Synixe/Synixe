class Cfgvehicles {
	class Plane_Fighter_01_Base_F; 
	class B_Plane_Fighter_01_F: Plane_Fighter_01_Base_F {
		class Components {
			class TransportPylonsComponent {
				class Presets {
					class GVAR(CAS) {
						thing[] = {"ace_maverick_L_PylonRack_x1","ace_maverick_L_PylonRack_x1","ace_maverick_L_PylonRack_x2","ace_maverick_L_PylonRack_x2","PylonMissile_Missile_BIM9X_x1","PylonMissile_Missile_BIM9X_x1","PylonMissile_Missile_AMRAAM_D_INT_x1","PylonMissile_Missile_AMRAAM_D_INT_x1","PylonRack_Bomb_SDB_x4","PylonRack_Bomb_SDB_x4","PylonMissile_Bomb_GBU12_x1","PylonMissile_Bomb_GBU12_x1"};
						displayName = "Synixe CAS";
					};
				};
				// class pylons {
				// 	class pylon1 {
				// 		attachment = "ace_maverick_L_PylonRack_x1";
				// 	};
				// 	class pylon2: pylon1 {
				// 		attachment = "ace_maverick_L_PylonRack_x1";
				// 	};
				// 	class pylon3 {
				// 		attachment = "ace_maverick_L_PylonRack_x2";
				// 	};
				// 	class pylon4: pylon3 {
				// 		attachment = "ace_maverick_L_PylonRack_x2";
				// 	};
				// 	class pylonBayCenter1 {
				// 		attachment = "PylonMissile_Missile_BIM9X_x1";
				// 	};
				// 	class pylonBayCenter2: pylonBayCenter1 {
				// 		attachment = "PylonMissile_Missile_BIM9X_x1";
				// 	};
				// 	class pylonBayCenter3 {
				// 		attachment = "PylonMissile_Missile_AMRAAM_D_INT_x1";
				// 	};
				// 	class pylonBayCenter4: pylonBayCenter3 {
				// 		attachment = "PylonMissile_Missile_AMRAAM_D_INT_x1";
				// 	};
				// 	class pylonBayCenter5 {
				// 		attachment = "PylonRack_Bomb_SDB_x4";
				// 	};
				// 	class pylonBayCenter6: pylonBayCenter5 {
				// 		attachment = "PylonRack_Bomb_SDB_x4";
				// 	};
				// 	class pylonBayLeft1: pylonBayRight1 {
				// 		attachment = "PylonMissile_Bomb_GBU12_x1";
				// 	};
				// 	class pylonBayRight1 {
				// 		attachment = "PylonMissile_Bomb_GBU12_x1";
				// 	};
				// };
			};
		};
	};
};
